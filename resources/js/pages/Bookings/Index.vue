...
const filteredBookings = computed(() => {
    ...
    return props.bookings.filter((booking) => {
        ...
        const itemsMatch = booking.items && booking.items.some((item) => item.menu.name.toLowerCase().includes(lowerSearchQuery));
        const tableMatch = booking.occupied_table && booking.occupied_table.table.name.toLowerCase().includes(lowerSearchQuery);
        const statusMatch = getBookingStatus(booking).toLowerCase().includes(lowerSearchQuery);

        return (
            customerName.includes(lowerSearchQuery) ||
            (booking.phone_number && booking.phone_number.toLowerCase().includes(lowerSearchQuery)) ||
            itemsMatch ||
            tableMatch ||
            statusMatch
        );
...
const getStatusVariant = (booking: BookingData): 'default' | 'destructive' | 'outline' | 'success' => {
        ...
        <div class="flex flex-col gap-6 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">View and manage restaurant bookings.</p>
                </div>
            ...
            <BookingNav :current-path="route('bookings.index')" />
                ...
                <div class="overflow-hidden rounded-lg border dark:border-gray-700">
                    <Table>
                        ...
                        <TableBody>
                                ...
                                <TableRow v-for="booking in filteredBookings" :key="booking.id" class="dark:border-gray-700">
                                    ...
                                    <TableCell class="dark:text-gray-300"
                                        >{{ booking.occupied_table?.table?.name || 'N/A' }} ({{
                                            booking.occupied_table?.table?.capacity ? booking.occupied_table.table.capacity + ' seats' : 'N/A'
                                        }})</TableCell
                                    >
                                    <TableCell class="dark:text-gray-300">
                            ...
                            <TableRow v-if="filteredBookings.length === 0" class="dark:border-gray-700">
                                <TableCell colspan="9" class="text-muted-foreground py-6 text-center dark:text-gray-400">
                                    No bookings found
                                </TableCell>
                            </TableRow>
                        </TableBody>
